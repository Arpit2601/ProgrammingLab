        ...
        while(running)
        {
            ...
            for(int i=0;i<4;i++)
            {
                synchronized (Main.BufferSockMatchingLocks[i])
                {
                    ...
                    {
                        System.out.println(Name + "removed 2 socks of color " + Main.GetSockColor(i+1) + " from SockMatching buffer.");
                        Main.BufferSockMatching[i]-=2;
                        synchronized (Main.BufferMatchingShelfLocks[i])
                        {
                            System.out.println(Name + " placed two socks of color " + Main.GetSockColor(i+1) + " on MatchingShelf buffer.");
                            Main.BufferMatchingShelf[i]+=2;
